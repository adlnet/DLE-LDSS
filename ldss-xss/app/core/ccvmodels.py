...
class FilterRecord(models.Model):
    ...
    def __simple_metadata_filter(self, queryset):
        ...
        if self.comparator in (self.EQUAL, self.CONTAINS):
...
class CCVUpstream(models.Model):
    ...
    metadata_experiences = models.ManyToManyField(MetadataLedger, related_name='ccv_source', blank=True)
    supplemental_experiences = models.ManyToManyField(SupplementalLedger, related_name='ccv_source', blank=True)
    ...
    def get_endpoint(cls):
        try:
            ...
            raise ValidationError('No CCV Upstream instance found') from e

class LCVDownstream(models.Model):
    """Model for Downstream LCV Communication """
    ACTIVE = 'ACTIVE'
    INACTIVE = 'INACTIVE'
    ...
    lcv_api_endpoint = models.CharField(
        max_length=200,
        help_text='Enter the LCV Instance API endpoint'
    )
    lcv_api_endpoint_status = models.CharField(max_length=200, choices=STATUS)

    lcv_api_key = models.CharField(
        help_text="Enter the LCV API Key",
        max_length=128
    )

    source_name = models.CharField(max_length=200, help_text='Enter the name to send data as')

    composite_experiences = models.ManyToManyField(CompositeLedger, related_name='lcv_destination', blank=True)

    filter_records = models.ManyToManyField(FilterRecord, related_name='lcv_downstream', blank=True)
    filter_metadata = models.ManyToManyField(FilterMetadata, related_name='lcv_downstream', blank=True)

    def __str__(self):
        """String for representing the Model object."""
        return f'{self.lcv_api_endpoint}'
    ...
    def apply_filter(self, queryset=CompositeLedger.objects.all()):
        ...
        queryset = queryset.filter(
            record_status='Active').exclude(lcv_destination__pk=self.pk)
    ...
    def get_endpoints(cls):
        try:
            endpoints = cls.objects.values_list('lcv_api_endpoint', flat=True)
            if endpoints:
            ...
            raise ValidationError('No LCV Downstream instance found') from e

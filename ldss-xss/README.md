...
# Prerequisites
`Python >=3.9`  *Download and install Python from here [Python](https://www.python.org/downloads/).*
...
These values will tell the Django application where to find its primary database, as well as the Neo4j database.
...
`NEO4J_USER` - The Neo4j Username (default: `neo4j`)

`NEO4J_PASSWORD` - The Neo4j Password (default: `password`)

`NEO4J_HOST` - Locally-resolvable hostname for the Neo4j intance (default: `neo4j`)

`NEO4J_PORT` - Locally-resolvable port for the Neo4j instance (default: `7687`)

`NEO4J_DATABASE` - Which database to use (default: `neo4j`)

`NEO4J_BOLT_URL` - The Bolt-protocol URL (default: `bolt://neo4j:password@neo4j:7687`)
...
`HOSTS` - Allowed hostnames for the application (default: `*;`)

`CORS_ALLOWED_ORIGINS` - Acceptable hostnames from which to allow CORS

`STYLE_SHA` - Sha256 hash of the CSS styling (optional)

`IMAGE_SHA` - Sha256 hash of the Images used by the client (optional)

### Deployment Config

`SERVER_NAME` - The public-facing hostname of this instance.

`INSTANCE_ID` - The instance ID associated with this instance's terms.

`TESTING` - Whether or not this is a testing instance (default: `true`)

`CCV` - Whether or not this is a CCV instance (default: `false`)
...
All of the refactor work to the data models currently exist in a branch called: "bay-models"

...
describe('neodefinitionpage', () => {
    beforeEach(() => {
      cy.wait(2000)
      const url = "/admin/core/neodefinition/"
      ...
      cy.contains('a', 'Neo definition').click();
    })

    it('Add Neo Definition +', () => {
        // Navigate to /core/neocontext/add
        cy.contains('a', 'Add neo definition').click()
    ...
    it('Add Neo Context', () => {
        ...
        // Enter embedding
        cy.get('input[name="embedding"]').type('testedding')

        // Check & uncheck rejected box
        cy.get('input[type="checkbox"]').check()
        cy.get('input[type="checkbox"]').uncheck()

        // Ensure definition is empty
        ...
        // Saving a Neo Context results in Server 500 error
    })

    // (Currently this results in a Server Error 500)
    // it('Delete Neo Definition', () => {
    ...
    //     // Select "Delete selected neo definitions" from the drop-down menu
    //     cy.get('.actions')
    //       .find('select[name="action"]')
    //       .select('Delete selected neo definitions')
